...
func (d *Downloader) Download(
	...
	// Validate inputs with enhanced error wrapping
	...
	if options.CreateDirs {
		...
		if _, err := os.Stat(parentDir); os.IsNotExist(err) {
			// Parent directory doesn't exist and CreateDirs is false
			downloadErr := errors.NewDownloadErrorWithDetails(
	...
	// Main download loop with retry logic and recovery
	var lastErr error
	...
	for attemptCount = 1; attemptCount <= d.retryManager.MaxRetries+1; attemptCount++ {
		...
		// Reset stats for this attempt (keep cumulative data)
		...
		// Download failed, analyze failure and generate recovery recommendations
		...
		if analysisErr != nil {
			...
			if len(recommendation.RecommendedActions) > 0 {
				// Add the top recommended action to previous actions
				previousActions = append(previousActions, recommendation.RecommendedActions[0].Type)
			}
		}

		// Check if we should retry
		if attemptCount >= d.retryManager.MaxRetries+1 {
		...
		if !errors.IsRetryable(err) {
			d.logError("non_retryable_error", err, map[string]interface{}{
				"attempt": attemptCount,
			})
	...
	// If the last error is non-retryable, return it directly to preserve the error code
	if lastErr != nil && !errors.IsRetryable(lastErr) {
		...
		if stdErrors.As(lastErr, &downloadErr) {
			...
			d.logError("download_final_failure", downloadErr, map[string]interface{}{
				"total_attempts": attemptCount - 1,
				"url":            url,
				"destination":    destination,
				"error_code":     downloadErr.Code.String(),
			})
	...
	// For retryable errors that exhausted all retries, preserve the original error details
	var finalErr *errors.DownloadError
	if lastErr != nil {
		downloadErr := &errors.DownloadError{}
		if stdErrors.As(lastErr, &downloadErr) {
			// Preserve the original error code and message
			finalErr = &errors.DownloadError{
				...
				Retryable:        false, // Mark as non-retryable after exhausting retries
			}
		} else {
		...
		// Fallback if no last error (shouldn't happen)
...
func (d *Downloader) performResumeDownload(
	...
	if err == nil {
		...
		// File exists, get its size for resume
		resumeOffset = fileInfoStat.Size()
		...
		return nil, errors.WrapErrorWithURL(err, errors.CodePermissionDenied, "Failed to check existing file for resume", url)
	}
	...
	// Check if we got partial content (206) or full content (200)
	...
	case http.StatusOK:
		...
		stats.Resumed = false // Not actually resumed

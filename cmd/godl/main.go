// Package main provides the command-line interface for the godl download tool.
package main
...
import (
	...
	godl "github.com/forest6511/godl"
...
func parseArgs() (*config, string, error) {
	...
	flag.StringVar(&cfg.userAgent, "user-agent", "godl/"+version, "User-Agent string to use")
...
func runPluginCommand(args []string) int {
	...
	case "install":
		if len(args) < 3 {
			...
			fmt.Fprintf(os.Stderr, "Usage: godl plugin install <source> <name>\n")
			return 1
		}
	...
	case "remove":
		if len(args) < 2 {
			...
			fmt.Fprintf(os.Stderr, "Usage: godl plugin remove <name>\n")
			return 1
		}
	...
	case "enable":
		if len(args) < 2 {
			...
			fmt.Fprintf(os.Stderr, "Usage: godl plugin enable <name>\n")
			return 1
		}
	...
	case "disable":
		if len(args) < 2 {
			...
			fmt.Fprintf(os.Stderr, "Usage: godl plugin disable <name>\n")
			return 1
		}
	...
	case "config":
		if len(args) < 4 || args[2] != "--set" {
			...
			fmt.Fprintf(os.Stderr, "Usage: godl plugin config <name> --set <key>=<value>\n")
			return 1
		}
...
func setupPlugins(ctx context.Context, downloader *godl.Downloader, cfg *config) error {
...
func handleStorageURL(ctx context.Context, downloader *godl.Downloader, storageURL string) error {
...
func setupS3Storage(ctx context.Context, downloader *godl.Downloader, parsedURL *url.URL) error {
...
func setupGCSStorage(ctx context.Context, downloader *godl.Downloader, parsedURL *url.URL) error {
...
func setupFileStorage(ctx context.Context, downloader *godl.Downloader, parsedURL *url.URL) error {
...
func performEnhancedDownload(
	...
	// Convert types.DownloadOptions to godl.Options
	godlOptions := &godl.Options{
...
Examples:
  %s plugin list
  %s plugin install github.com/user/godl-plugin-s3 s3
  %s plugin remove s3
...
Download Options:
  -o, --output FILE        Output filename (default: extract from URL)
      --user-agent STRING  User-Agent string to use (default: godl/%s)
...
Plugin Options:
      ...
      --plugin-dir DIR    Plugin directory (default: ~/.godl/plugins)
      --plugin-config FILE Plugin configuration file (default: ~/.godl/plugins.json)
...
Plugin Management Examples:
  ...
  %s plugin install github.com/user/godl-plugin-s3 s3        # Install S3 plugin
